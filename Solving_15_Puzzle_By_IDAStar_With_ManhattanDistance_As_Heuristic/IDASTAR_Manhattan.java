//This code takes a sequence of 15 puzzle and outputs 
//the steps to reach goal by following IDA* search Algorithm using
...
class StateNodeIDMan {
	int[] state;
	StateNodeIDMan parent;
	int depth;
	int heuristic;
}

//Custom comparator
class HeuristicComparatorAstarMan implements Comparator<StateNodeIDMan>
{
    @Override
    public int compare(StateNodeIDMan x, StateNodeIDMan y)
    {
...
public class IDASTAR_Manhattan {
	private static final StateNodeIDMan NULL = null;
	...
	public boolean findPath(int[] state, int depth) {
		...
		Comparator<StateNodeIDMan> comparator = new HeuristicComparatorAstarMan();
        PriorityQueue<StateNodeIDMan> toexplore = 
            new PriorityQueue<StateNodeIDMan>(100, comparator);
		StateNodeIDMan start_state = new StateNodeIDMan();
		StateNodeIDMan next_state = new StateNodeIDMan();
		...
		while (!toexplore.isEmpty()) {
			...
			StateNodeIDMan temp_state = new StateNodeIDMan();
...
//			System.out.print(goal[index]+", ");
	...
	public void printPath(StateNodeIDMan curr_state) {
	...
	public static void main(String[] args) throws IOException {
		...
		IDASTAR_Manhattan bfs = new IDASTAR_Manhattan();
